        ...
        while (openNodes.Count > 0)
        {
            ...
            foreach (var adjNode in grid.GetAdjacentLocations(node))
            {
                ...
                if (newCostToNeighbour < adjNode.G || !openNodes.Contains(adjNode))
                {
                    ...
                    adjNode.H = !adjNode.Expensive ? GetTraversalCost(adjNode, targetNode) : Mathf.RoundToInt((grid.GridWorldSize.x * grid.GridWorldSize.y)*.8f) + GetTraversalCost(adjNode, targetNode);
                    adjNode.Parent = node;
